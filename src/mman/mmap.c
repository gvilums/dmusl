...
void *__mmap(void *start, size_t len, int prot, int flags, int fd, off_t off) {
	extern char _end;
	static uintptr_t __mmap_base_addr = 0;
	static uintptr_t __mmap_end_addr = 0;

	if (__mmap_base_addr == 0) {
		const size_t map_size = 1ull << 30; // 1GiB heap for now
		__mmap_base_addr = PAGE_ALIGN((uintptr_t)&_end);
